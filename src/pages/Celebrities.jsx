...
//   const [celebritiesTrend, setCelebritiesTrend] = useState([]); // Change naming to reflect celebrities
//   const [regularCelebritiesItems, setRegularCelebritiesItems] = useState([]); // Change naming to reflect celebrities
//   const [recommendedCelebritiesItems, setRecommendedCelebritiesItems] =
//     useState([]); // Change naming to reflect celebrities
//   const [news, setNews] = useState([]);

//   const fetchCelebritiesData = useCallback(async (category, setData) => {
//     try {
//       const response = await fetch(
//         `${backendURL}/api/getAllFashion?category=${category}&postType=Celebrities`
//       );
//       if (!response.ok) {
...
//         await Promise.all([
//           fetchNews(),
//           fetchCelebritiesData("TopTrend", setCelebritiesTrend),
//           fetchCelebritiesData("Regular", setRegularCelebritiesItems),
//           fetchCelebritiesData("Recommended", setRecommendedCelebritiesItems),
//         ]);
//       } catch (error) {
//         setError("Failed to load celebrities data");
//       } finally {
//         setLoading(false);
//       }
//     };
...
//     loadData();
//   }, [fetchNews, fetchCelebritiesData]);
...
//           Welcome to the Celebrities section! Here, we bring you the latest
//           news, gossip, and updates about your favorite stars. Stay tuned for
//           exclusive interviews, red carpet events, and more!
//         </h3>

//         {/* TrendCard */}
//         <div className="container mx-auto px-4 py-12">
//           <div className="grid grid-cols-1 gap-8">
//             {celebritiesTrend.map((celebrity) => (
//               <TrendCard key={celebrity._id} trend={celebrity} />
//             ))}
//           </div>
//         </div>
//         {/* TrendCard */}
...
//             {regularCelebritiesItems.map((item) => (
...
//         <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 lg:gap-6">
//           {recommendedCelebritiesItems.map((item) => (
...
const Celebrities = () => {
  ...
  const fetchSociety = useCallback(async () => {
    try {
      setLoading(true);
      const response = await fetch(
        `${backendURL}/api/getAllFashion?postType=Celebrities`
      );
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <main className="lg:w-2/3">
            <div className="ml-4 my-5">
              <h3 className="text-xl font-normal">Category : Celebrities</h3>
            </div>
