...
//   const [travelTrend, setTravelTrend] = useState([]);
//   const [regularTravelItems, setRegularTravelItems] = useState([]);
//   const [recommendedTravelItems, setRecommendedTravelItems] = useState([]);
//   const [news, setNews] = useState([]);

//   const fetchTravelData = useCallback(async (category, setData) => {
//     try {
//       const response = await fetch(
//         `${backendURL}/api/getAllFashion?category=${category}&postType=LifeStyle&subCategory=Travel`
//       );
//       if (!response.ok) {
...
//         await Promise.all([
//           fetchNews(),
//           fetchTravelData("TopTrend", setTravelTrend),
//           fetchTravelData("Regular", setRegularTravelItems),
//           fetchTravelData("Recommended", setRecommendedTravelItems),
//         ]);
//       } catch (error) {
...
//           Welcome to the Travel section! Here, we provide tips, guides, and
//           inspiration for your next adventure. From travel hacks to destination
//           highlights, we have everything you need to plan your perfect getaway!
//         </h3>

//         {/* TrendCard */}
//         <div className="container mx-auto px-4 py-12">
//           <div className="grid grid-cols-1 gap-8">
//             {travelTrend.map((travel) => (
//               <TrendCard key={travel._id} trend={travel} />
//             ))}
//           </div>
//         </div>
//         {/* TrendCard */}
...
//         <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 lg:gap-6">
//           {recommendedTravelItems.map((item) => (
...
const Travel = () => {
  ...
  const fetchTravel = useCallback(async () => {
    try {
      setLoading(true);
      const response = await fetch(
        `${backendURL}/api/getAllFashion?postType=LifeStyle&subCategory=Travel`
      );
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <main className="lg:w-2/3">
            <div className="ml-4 my-5">
              <h3 className="text-xl font-normal">Category : Travel</h3>
            </div>
