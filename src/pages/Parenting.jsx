...
const Parenting = () => {
  ...
  const fetchParenting = useCallback(async () => {
    try {
      setLoading(true);
      const response = await fetch(
        `${backendURL}/api/getAllFashion?postType=LifeStyle&subCategory=Parenting`
      );
      if (!response.ok) throw new Error("Failed to fetch Parenting data");
      const data = await response.json();
      setNewsItems(data.posts);
      setError(null);
    } catch (error) {
      console.error("Error fetching Parenting data:", error);
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <main className="lg:w-2/3">
            <div className="ml-4 my-5">
              <h3 className="text-xl font-normal">Category : Parenting</h3>
            </div>
