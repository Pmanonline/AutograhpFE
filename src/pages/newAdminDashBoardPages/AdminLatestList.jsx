...
export default function LatestList() {
    ...
    async (startIndex = 0) => {
      try {
        const res = await fetch(
          `${backendURL}/api/getAllLatest?startIndex=${startIndex}&limit=9`
        );
        ...
        showSnackbar(`Failed to fetch latest posts: ${error.message}`, "error");
      } finally {
        setIsLoading(false);
      }
    },
  ...
  const handleDeletePost = async () => {
    ...
    try {
      const response = await fetch(
        `${backendURL}/api/deleteLatest/${postIdToDelete}`,
        { method: "DELETE" }
      );

      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.message || "Failed to delete latest post");
      }
      ...
      showSnackbar("Latest post deleted successfully", "success");
      setDeleteOpen(false);
    } catch (error) {
      console.error("Error deleting latest post:", error);
      showSnackbar(error.message || "Failed to delete latest post", "error");
    } finally {
      setIsDeleting(false);
      setPostIdToDelete("");
    }
    ...
    return latestPosts.map((post) => (
      <TableRow key={post._id} hover>
        ...
        <StyledTableCell>
          <Button
            component={Link}
            to={`/DashBoard/Admin/CreateLatestPost/${post._id}`}
            variant="contained"
            color="primary"
            size="small"
          >
  ...
  return (
    <Paper sx={{ width: "100%", overflow: "hidden" }}>
      <Button
        component={Link}
        to="/DashBoard/Admin/CreateLatestPost"
      ...
      <TableContainer sx={{ maxHeight: 500 }}>
        <Table stickyHeader aria-label="latest posts table">
          <TableHead>
            <TableRow>
